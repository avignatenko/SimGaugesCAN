...
class Heading2(cansimlib.Device2):
    ...
    async def _run_heading_update(self):
        dataref = await self.create_dataref_subscription(
            "sim/cockpit/gyros/psi_vac_ind_degm", tolerance=0.1
        )
    ...
    async def _run_ap_bug_update(self):
        dataref = await self.create_dataref_subscription(
            "sim/cockpit/autopilot/heading_mag", tolerance=0.1
        )
    ...
    async def _run_dg_drift_knob_update(self):
        ...
        dg_drift_dataref_id = await self._sim.get_dataref_id(
            "sim/cockpit/gyros/dg_drift_vac_deg"
        )
        ...
        while True:
            ...
            logger.debug("update sent!! %s", self._dg_drift_current)
            self._dg_drift_restore_sim_mode_task = asyncio.create_task(
                self._restore_sim_mode_dg_drift_mag()
            )
    ...
    async def _run_dg_ap_bug_knob_update(self):
        ...
        ap_bug_mag_dataref_id = await self._sim.get_dataref_id(
            "sim/cockpit/autopilot/heading_mag"
        )
        ...
        while True:
            ...
            await self._sim.send_dataref(
                ap_bug_mag_dataref_id, None, self._ap_bug_mag_current
            )
            logger.debug("update sent!! %s", self._ap_bug_mag_current)
            self._ap_bug_restore_sim_mode_task = asyncio.create_task(
                self._restore_sim_mode_ap_bug_mag()
            )
